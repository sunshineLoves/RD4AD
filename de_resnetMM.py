...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.memory1 = MemoryModule(L=embed_dim, channel=256)
        self.memory2 = MemoryModule(L=embed_dim, channel=512)
        self.memory3 = MemoryModule(L=embed_dim, channel=1024)
    ...
    def _forward_impl(self, x: List[Tensor]) -> List[Tensor]:
        ...
        #x = torch.flatten(x, 1)
        #x = self.fc(x)
        feature_a = self.layer1(torch.cat([x[2], x[2]], dim=1))
        b, c, h, w = feature_a.size()
        feature_a_half = feature_a.reshape(b, c // 2, 2, h, w)
        feature_a_half = feature_a_half.mean(dim=2)
        feature_a_cat = torch.cat([feature_a_half, x[1]], dim=1)
        feature_b = self.layer2(feature_a_cat)
        b, c, h, w = feature_b.size()
        feature_b_half = feature_b.reshape(b, c // 2, 2, h, w)
        feature_b_half = feature_b_half.mean(dim=2)
        feature_b_cat = torch.cat([feature_b_half, x[0]], dim=1)
        feature_c = self.layer3(feature_b_cat)

        feature_c = F.avg_pool2d(feature_c, kernel_size=2, stride=2)
        feature_b = F.avg_pool2d(feature_b, kernel_size=2, stride=2)
        feature_a = F.avg_pool2d(feature_a, kernel_size=2, stride=2)
...
def _resnet(
    ...
    **kwargs: Any
    ...
    model = ResNet(block, layers, embed_dim=100, **kwargs)
    if pretrained:
